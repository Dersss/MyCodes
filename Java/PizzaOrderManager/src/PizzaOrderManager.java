...
public final class PizzaOrderManager {
    ...
    private static void getPriceMap(String fileName,
        ...
        while (!in.atEOS()) {
            ...
            int comma = hold.indexOf(",");
            String word = hold.substring(0, comma);
            String price = hold.substring(comma + 1);
            priceMap.add(word, Integer.parseInt(price));
        }
    ...
    private static int getOneOrder(SimpleReader input,
        ...
        while (!test.equals("")) {
            test = input.nextLine();
            if (sizePriceMap.hasKey(test)) {
                price += sizePriceMap.value(test);
            }
            if (toppingPriceMap.hasKey(test)) {
                price += toppingPriceMap.value(test);
            }
    ...
    private static void putPrice(SimpleWriter output, int price) {
        ...
        int first = price / 100;
        int second = price % 100;
        if (second > 9) {
            output.println("$" + first + "." + second);
        } else {
            output.println("$" + first + "." + second + "0");
        }
    }
