...
public class Map2<K, V> extends MapSecondary<K, V> {
    ...
    private static <K, V> void moveToFront(Queue<Pair<K, V>> q, K key) {
        ...
        while (i < q.length() && !end) {
            Map.Pair<K, V> itemInTest = q.dequeue();
            if (itemInTest.key().equals(key)) {
                q.enqueue(itemInTest);
                q.flip();
                end = true;
            } else {
                q.enqueue(itemInTest);
    ...
    public final void add(K key, V value) {
        ...
        Pair<K, V> toAdd = new SimplePair<>(key, value);
        this.pairsQueue.enqueue(toAdd);
    ...
    public final Pair<K, V> remove(K key) {
        ...
        Map.Pair<K, V> toRemove = this.pairsQueue.dequeue();
        if (!toRemove.key().equals(key)) {
            this.pairsQueue.enqueue(toRemove);
        }
        return toRemove;
    }
