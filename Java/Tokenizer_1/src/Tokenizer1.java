...
public final class Tokenizer1 {
    ...
    private static String nextWordOrSeparator(String text, int position) {
        ...
        // TODO - fill in body
        int index = position;
        int testIndex = position + 1;
        String end = "";
        String test = "";
        test += text.charAt(position);
        end += text.charAt(position);
        boolean sep = SEPARATORS.contains(test);
        boolean change = false;
        if (sep) {
            while (testIndex < text.length() && !change) {
                if (SEPARATORS.indexOf(text.charAt(testIndex)) != -1) {
                    testIndex++;
                } else {
                    change = true;
                }
        ...
        return text.substring(position, testIndex);
    }
    ...
    public static Queue<String> tokens(SimpleReader in) {
        ...
        while (!in.atEOS()) {
            ...
            while (position < hold.length()) {
                String add = nextWordOrSeparator(hold, position);
                position += add.length();
                if (SEPARATORS.indexOf(add.charAt(0)) == -1) {
                    tokens.enqueue(add);
                }
            }
    ...
    public static void main(String[] args) {
        ...
        Queue<String> q2 = Tokenizer1.tokens(file);
        file.close();
        /*
