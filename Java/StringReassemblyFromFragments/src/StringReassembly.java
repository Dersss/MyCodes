...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        /**
         * Returns substring with specified overlap.
         */
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        // Boolean to track whether or not to add str to strSet
        boolean add = true;

        /**
         * For-each loop going through each item in Set and checking to see if
         * str is a substring of any items of the set. If so, boolean add is
         * given a value of false.
         */
        for (String x : strSet) {
            String pull = x;
            /**
             * if index is -1 then String pull does not contain str.
             */
            if (pull.indexOf(str) != -1) {
                add = false;
            }
        }

        /**
         * If none of the values of the Set have str as a substring, loop
         * through the set to see if any of the items of the set are substrings
         * of str.
         */
        if (add) {
            Set<String> temp = strSet.newInstance();
            while (strSet.size() > 0) {
                String pull2 = strSet.removeAny();

                /**
                 * If value is -1 then pull2 is not a substring so it should be
                 * added back to the set.
                 *
                 * If the value is equal to 1 then pull2 is included in the str
                 * to be added. So oull2 is not added back
                 */
                if (str.indexOf(pull2) == -1) {
                    temp.add(pull2);
                }
            }
            /**
             * Adds str to temp and then transfers temp to strSet
             */
            temp.add(str);
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        /**
         * Adds strings to set while the input is not at the End Of Stream.
         */
        Set<String> toReturn = new Set1L<>();
        while (!input.atEOS()) {
            String line = input.nextLine();

            /**
             * Adds to set avoiding substrings
             */
            addToSetAvoidingSubstrings(toReturn, line);
        }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        if (text.contains("~")) {
            String new1 = text.replaceAll("~", System.lineSeparator());
            out.println(new1);
        } else {
            out.println(text);
        }
