 ...
 *
 */
public final class OLD_CryptoUtilities {

    /**
     * Private constructor so this utility class cannot be instantiated.
     */
    private OLD_CryptoUtilities() {
    }
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        while (!m.isZero()) {
            ...
            int m2 = m.toInt();
            int mod = n2 % m2;
            NaturalNumber newM = new NaturalNumber2(mod);
            n.copyFrom(temp);
            m.copyFrom(newM);
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        NaturalNumber nTemp = new NaturalNumber2(1);
        NaturalNumber nStay = new NaturalNumber2(n);
        NaturalNumber pTemp = new NaturalNumber2(p);
        NaturalNumber mTemp = new NaturalNumber2(m);
        pTemp.divide(new NaturalNumber2(2));

        for (int i = 0; i < (pTemp.toInt()); i++) {
            nTemp.multiply(nStay);
        }
        if (pTemp.compareTo(new NaturalNumber2(1)) == 0) {
            nTemp.multiply(nStay);
        }
        NaturalNumber div = new NaturalNumber2(nTemp);
        div.divide(mTemp);

        NaturalNumber mod = new NaturalNumber2(div);

        mod.multiply(m);

        NaturalNumber sub = new NaturalNumber2(nTemp);
        sub.subtract(mod);
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        nDec.decrement();
        powerMod(wTemp1, new NaturalNumber2(2), n);

        if (wTemp1.compareTo(new NaturalNumber2(1)) == 0) {
            return true;
        }
        powerMod(wTemp2, nDec, n);
        if (wTemp2.compareTo(new NaturalNumber2(1)) != 0) {
            return true;
        } else {
            return false;
        }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        NaturalNumber nDec = new NaturalNumber2(n);
        nDec.decrement();
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0 || even) {
            isPrime2 = true;
        } else {
            NaturalNumber randomChecks = randomNumber(nDec);
            if (randomChecks.compareTo(new NaturalNumber2(1)) != 1) {
                randomChecks.increment();
            }

            while (counter <= 50) {
                if (randomChecks.compareTo(new NaturalNumber2(1)) == 1
                        && randomChecks.compareTo(nDec) == -1) {
                    boolean composite = isWitnessToCompositeness(randomChecks,
                            n);
                    if (composite) {
                        isPrime2 = false;
                    }
