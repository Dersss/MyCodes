...
public final class StringReassembly_OLD {
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        for (String tempr : temp) {
            if (tempr.indexOf(str) != -1) {
                strFound = true;
            }
        }
        if (!strFound) {
            int size = temp.size();
            while (size > 0) {
                String hold2 = temp.removeAny();
                if (str.indexOf(hold2) == -1) {
                    strSet.add(hold2);
                }
                size--;
            }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        while (!input.atEOS()) {
            test = input.nextLine();
            addToSetAvoidingSubstrings(strSet2, test);
