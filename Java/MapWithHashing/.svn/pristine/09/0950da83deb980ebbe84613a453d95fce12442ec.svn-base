...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    private void createNewRep(int hashTableSize) {
        ...
        while (pos < hashTableSize) {
            this.hashTable.setEntry(pos, new Map2<K, V>());
            pos++;
        }
        this.size = 0;
    ...
    public final void add(K key, V value) {
        ...
        int index = mod(hashVal, this.hashTable.length());
    ...
    public final Pair<K, V> remove(K key) {
        ...
        int pos = 0;
        while (!this.hashTable.entry(pos).hasKey(key)) {
            pos++;
        }
        this.size--;

        return this.hashTable.entry(pos).remove(key);
    }
    ...
    public final boolean hasKey(K key) {
        ...
        for (int i = 0; i < this.hashTable.length() && !found; i++) {
            if (this.hashTable.entry(i).hasKey(key)) {
                found = true;
            }
        }
