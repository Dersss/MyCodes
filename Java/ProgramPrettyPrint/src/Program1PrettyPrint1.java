...
public final class Program1PrettyPrint1 extends Program1 {
    ...
    public void prettyPrint(SimpleWriter out) {
        ...
        final int oneIndent = 4;
        final int twoIndent = 8;

        Map<String, Statement> s1 = this.newContext();
        Map<String, Statement> cond = this.replaceContext(s1);

        out.println("PROGRAM " + this.name() + " IS");
        out.println();

        while (cond.size() > 0) {
            Map.Pair<String, Statement> tmp = cond.removeAny();
            String name = tmp.key();
            Statement body = tmp.value();

            printSpaces(out, oneIndent);
            out.println("INSTRUCTION " + name + " IS");
            tmp.value().prettyPrint(out, twoIndent);
            s1.add(name, body);
        ...
        this.replaceContext(s1);
        Statement body = this.newBody();
        Statement tmpBody = this.replaceBody(body);
        out.println("BEGIN");
        tmpBody.prettyPrint(out, oneIndent);
        out.println("END" + this.name());
        this.replaceBody(tmpBody);
