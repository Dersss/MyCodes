...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
        ...
        if (last >= 1) {
            ...
            if (left != 0) {
                ...
                if (order.compare(array.entry(top),
                        array.entry(smallestChild)) > 0) {
                    array.exchangeEntries(top, smallestChild);
                    siftDown(array, smallestChild, last - 1, order);
                }
            }
        }
    }
    ...
    private static <T> void heapify(Array<T> array, int top,
        ...
        int chld1 = 2 * top + 1;
        int chld2 = 2 * top + 2;
        if (chld1 < array.length() - 1
                && order.compare(array.entry(chld1), array.entry(small)) < 0) {
            small = chld1;
        }
        if (chld2 < array.length() - 1
                && order.compare(array.entry(chld2), array.entry(small)) < 0) {
            small = chld2;
        }
        if (small != top) {
            array.exchangeEntries(top, small);
            heapify(array, array.length() - 1, order);
        }
        if (isHeap(array, 2 * top + 1, array.length() - 1, order)
                && isHeap(array, 2 * top + 2, array.length() - 1, order)) {
    ...
    private static <T> Array<T> buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        // TODO - fill in body
        Array<T> toRet = new Array1L<>(q.length());
        while (q.length() > 0) {
            T hold = q.dequeue();
            toRet.setEntry(q.length(), hold);
        }
        heapify(toRet, 0, order);
