...
public final class CryptoUtilities {
    ...
    /**
     * Constant for generating 50 random test cases.
     */
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        /**
         * If m is not zero, we have not found the GCD of both NN's. Creates a
         * NaturalNumber mod to hold the modulus of n and m. Recursively calls
         * GCD(m, n) until the 2nd value is zero.
         */
        if (!m.isZero()) {
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        /**
         * Creates NN values for a temporary n and one to hold the modulus of n
         * and 2. If mod is zero, then the number is even.
         */
        boolean end = false;
        NaturalNumber nTemp = new NaturalNumber2(n);
        NaturalNumber mod = nTemp.divide(new NaturalNumber2(2));
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        /**
         * if p iz zero then the value of n is set to 0. Any number raised to
        ...
        if (p.isZero()) {
            ...
            /**
             * If p is not zero, creates temporary values of n and p.
             */
            NaturalNumber nCopy = new NaturalNumber2(n);

            /**
             * If p is even, method follows the algorithm: ( n ^ (p/2) )^2 mod m
             *
             *
             * If p is odd, method follows the algorithm: (n * (n^(p-1/2)^2))
             * mod m
             *
             */
            if (isEven(p)) {
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        /**
         * Creates two test values for w and one for the decremented value of n.
         *
         */
        ...
        /**
         * Calls power mod to check for cases states in @ensures clause of
         * contract.
         *
         */
        powerMod(wCopy1, new NaturalNumber2(2), n);
        powerMod(wCopy2, nCopy, n);

        /**
         * If first copy of w after powerMod comes back as 1 or second copy of w
         * comes back as something other than one than w is a witness that n is
         * composite.
         */
        if (wCopy1.equals(new NaturalNumber2(1))
                || !wCopy2.equals(new NaturalNumber2(1))) {
            end = true;
        }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            /**
             * Creates 50 random test cases and calls the
             * isWitnessToCompositeness() method to see if the random number is
             * a witness that n is composite. Checks to make sure that values do
             * not break contract of method being called.
             */

            /**
             * Preconditions for isWitnessToCompositeness() n > 2 and 1 < w < n
             * - 1
             */
            ...
            while (index < FIFTY) {
                ...
                if (n.compareTo(new NaturalNumber2(2)) > 0
                        && random.compareTo(new NaturalNumber2(1)) > 0
                        && random.compareTo(nTemp) < 0) {
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        /**
         * Checks to whether initial value is prime, if not then recursively
         * calls itself while incrementing through the odd numbers.
         */

        boolean origPrime = isPrime2(n);
