...
public final class JCFExplorations {
    ...
    public static void giveRaise(components.map.Map<String, Integer> map,
        ...
        while (map.size() > 0) {
            components.map.Map.Pair<String, Integer> transferPair = map
                    .removeAny();
            String key = transferPair.key();
            int val = transferPair.value();
            double raise = 0;
            if (key.charAt(0) == initial) {
                raise = val * raisePercent / 100;
                val += raise;
            }
            temp.add(key, val);
        }
    ...
    public static void giveRaise(java.util.Map<String, Integer> map,
        ...
        Map<String, Integer> moveBack = new HashMap<>();
        for (Map.Entry<String, Integer> mP : temp) {
            String oneName = mP.getKey();
            int value = mP.getValue();
            if (oneName.charAt(0) == initial) {
                int raise = value * raisePercent / 100;
                value = value + raise;
            }
            moveBack.put(oneName, value);
        }
        map.putAll(moveBack);
    ...
    public static void incrementAll(components.set.Set<NaturalNumber> set) {
        ...
        // TODO - fill in body
        components.set.Set<NaturalNumber> copy = set.newInstance();
        while (set.size() > 0) {
            NaturalNumber hold = set.removeAny();
            hold.increment();
            copy.add(hold);
        }
        set.transferFrom(copy);
    }
    ...
    public static void incrementAll(java.util.Set<NaturalNumber> set) {
        ...
        // TODO - fill in body
        java.util.Set<NaturalNumber> copy = new HashSet<>();
        ...
        while (it.hasNext()) {
            ...
            num.increment();
            copy.add(num);
            System.out.println("After add: " + copy);
        }
        set.addAll(copy);
    }
