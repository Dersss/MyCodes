 ...
 *
 */
public final class ExpressionEvaluator2 {
    ...
    private static int valueOfTerm(StringBuilder source) {
        ...
        while (source.length() > 0
            ...
            if (source.charAt(0) == '*') {
                source.deleteCharAt(0);
                int next = valueOfFactor(source);
                result = result * next;
            } else {
                source.deleteCharAt(0);
                int next2 = valueOfFactor(source);
                result = result / next2;
            }
    ...
    public static int valueOfExpr(StringBuilder source) {
        ...
        while (source.length() > 0
            ...
            if (source.charAt(0) == '+') {
                source.deleteCharAt(0);
                int next = valueOfTerm(source);
                result = result + next;
            } else {
                source.deleteCharAt(0);
                int next2 = valueOfTerm(source);
                result = result - next2;
            }
